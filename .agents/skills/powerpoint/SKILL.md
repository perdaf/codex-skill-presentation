...
- For `PRESENTATION_FORMAT=PPTX`, use PptxGenJS as the primary engine and keep titles, text, shapes, tables, charts, simple diagrams, lines, and practical icons editable. For `HTML`, use the Web Presentation Engine and standards-based local files. For `GAMMA`, run `CONTEXT → INTENT → RESEARCH → TEACHING ENGINE → CONTENT ARCHITECTURE → VISUAL INTELLIGENCE`, then use `assets/gamma-pipeline.js` and `assets/gamma-export.js` to create the blueprint; do not ask Gamma to invent the pedagogy, call Gamma, upload content, or claim to have inspected the final Gamma rendering.
- A Gamma generation may contain at most 20 slides for this user's account. Build the complete pedagogical architecture first, then condense it without arbitrary truncation. Preserve objectives, prerequisites, demonstrations, essential procedures, practice, common errors, validation, and synthesis. Return `GAMMA_BLUEPRINT_INCOMPLETE` when the final blueprint is under-specified or still exceeds 20 slides.
- Every Gamma slide must contain its pedagogical role, exact displayed title and content, essential message, Visual Intelligence decision, precise visual direction, layout, interaction/progression when useful, and Gamma rendering instructions. A simple outline, placeholders, generic image requests, or a source-dominant document is not a valid deliverable.
- In Gamma blueprints, whenever people are genuinely useful, default to contemporary, varied, non-stereotypical Afro-Antillean representation unless the user explicitly requests another representation or no people. Preserve `AUDIENCE_REPRESENTATION=MARTINIQUE` exactly when the active context supplies it.
...
7. **Build:** for PPTX, compose projection slides with existing theme-aware [components](assets/presentation-components.js) and [layouts](assets/layouts.js) and follow [PptxGenJS conventions](references/pptxgenjs-conventions.md). For HTML, use `assets/web-presentation-engine.js`; keep each slide a 16:9 pedagogical screen and all runtime resources local. For Gamma, call `prepareGammaBlueprint()` from `assets/gamma-pipeline.js` after research and content architecture, then render `gamma-blueprint.md` and the separate `sources.md`. Build handouts as genuine A4 pages rather than slide printouts.
8. **Validate and improve:** run pedagogical validation before visual validation. For HTML, run `assets/web-validation.js`, browser interaction checks, and visual inspection. For Gamma, run `validateGammaBlueprint()` and block on missing concrete content, objective/competency/topic coverage, incorrect pedagogical order, placeholders, generic visuals, missing Brand/Senior/representation constraints, embedded sources, or more than 20 slides; final visual QA remains external because Gamma performs the rendering. For handouts, also validate print format and autonomy. Correct sources and rerender, retaining the existing three-pass maximum.
...
For `DUAL + HTML`, keep `presentation-web/`, `handout.pdf`, `handout.js`, and `content/course-content.md` in the same course project. For `DUAL + PPTX`, retain the established PPTX + PDF structure. For `DUAL + GAMMA`, keep `presentation-gamma/gamma-blueprint.md`, `presentation-gamma/sources.md`, the A4 handout sources/output, and `content/course-content.md` together.
