...
V4.6 leaves `resolveRequestWithContext()` unchanged and adds `resolveV46RequestWithContext()` as an additive wrapper. Context still resolves before presentation format. Neither the EPN context nor a preset may silently choose HTML or PPTX when usage remains ambiguous.
